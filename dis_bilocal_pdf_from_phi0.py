#!/usr/bin/env python3
"""
Option #3 (Parton / DIS) – POC bilocal (connected) correlator -> x-like spectrum from a boosted S^3 field snapshot.
...
Pipeline:
  ...
  3) Measure O = |∂_z Phi|^2 (representation-invariant scalar).
  4) Build 1D connected bilocal correlator along z of O1(z)=<O(x,y,z)>_{x,y} via Wiener–Khinchin,
     then map k_z -> x = k_z / Pz_est, save JSON + binned plot.

Notes:
  - Diagnostic only (no time evolution yet).
  - Uses vacuum blending outside boosted support to reduce FFT ringing.
"""
...
def estimate_core_width_along_z(phi: torch.Tensor, Phi_vac: torch.Tensor, dx: float, thresh: float = 0.1) -> float:
    """
    Estimate rest-frame core width along z at the central line (x=y=mid) using indicator 1 - Phi·Phi_vac > thresh.
    Returns width in physical units.
    """
    N = phi.shape[0]
    mid = N // 2
    dot0 = (phi[mid, mid, :, :] * Phi_vac).sum(dim=-1)  # (N,)
...
def estimate_vacuum_from_boundary(phi: torch.Tensor, shell: int = 3) -> torch.Tensor:
    """
    Estimate Phi_vac by averaging Phi on a boundary shell of thickness `shell`.
    Returns a unit 4-vector (tensor shape (4,)).
    """
    N = phi.shape[0]
    s = int(max(1, shell))
...
def resample_boost_contract_z(phi_rest: torch.Tensor, gamma: float, Phi_vac: torch.Tensor) -> torch.Tensor:
    """
    Lorentz-contracted snapshot at t=0 for rest-static profile:
      Phi_boost(x,y,z) = Phi_rest(x,y,z' = gamma*z)

    Uses torch grid_sample (trilinear) on normalized coords [-1,1]^3.
    Blends to Phi_vac near |Z| ~ 1/gamma (in OUTPUT coords).
    """
    ...
    # Smooth vacuum blend near boosted support edge |Z| ~ 1/gamma (OUTPUT coords)
    Phi_vac4 = Phi_vac.view(1, 1, 1, 4).to(device=device, dtype=dtype)
    zabs = Z.abs()
    ...
    phi_boost = normalize_S3(w * phi_boost + (1.0 - w) * Phi_vac4)
    return phi_boost


def energy_rest(phi: torch.Tensor, F0: float, lam: float, m: float, Phi_vac: torch.Tensor, dx: float) -> float:
    """
    Rest energy consistent with interior central diffs:
    e2 = (F^2/2) Tr X
    ...
    dot0 = (Phi_int * Phi_vac).sum(dim=-1)
...
def main():
    ...
    ap.add_argument("--vac_shell", type=int, default=3, help="Boundary shell thickness (in grid points) for Phi_vac estimate.")
    args = ap.parse_args()
    ...
    Phi_vac = estimate_vacuum_from_boundary(phi, shell=args.vac_shell).to(device=device, dtype=dtype)

    if args.gamma < 1.0:
        raise ValueError("--gamma must be >= 1")

    w0 = estimate_core_width_along_z(phi, Phi_vac, dx, thresh=0.1)
    v = math.sqrt(max(0.0, 1.0 - 1.0 / (args.gamma * args.gamma)))
    wz = w0 / args.gamma if w0 > 0 else 0.0
    npts = (wz / dx) if (wz > 0 and dx > 0) else 0.0

    E0 = energy_rest(phi, args.F0, args.lam, args.m, Phi_vac, dx)
    ...
    phi_boost = resample_boost_contract_z(phi, args.gamma, Phi_vac)

    dphi_dz = central_diff_z(phi_boost, dx)          # (N,N,N-2,4)
    O = (dphi_dz * dphi_dz).sum(dim=-1)              # (N,N,N-2)

    Nz = O.shape[2]
    # Window+FFT along z per (x,y), then average power over x,y
    win = torch.hann_window(Nz, device=O.device, dtype=O.dtype).view(1, 1, Nz)
    Ow = (O - O.mean(dim=2, keepdim=True)) * win
    Fk = torch.fft.rfft(Ow, dim=2)
    Pk = (Fk.conj() * Fk).real.mean(dim=(0, 1)).detach().cpu().numpy()
    ...
    payload = {
        ...
        "note": "Pk = |FFT(O1w)|^2 where O1(z)=<|∂zΦ|^2>_{x,y} from a contracted snapshot; connected mean-subtracted; Hann window applied.",
    }
